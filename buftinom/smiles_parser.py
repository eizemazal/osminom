...
from buftinom.smileg import Atom, Bond, Molecule
...
class SmilesParser:
    ...
    def p_molecule(self, p):
        """molecule : atom"""
        _, atom = p
        p[0] = Molecule().add_atom(atom)

    def p_molecule_atom(self, p: tuple[Molecule, Molecule, Atom]):
        """molecule : molecule atom"""
        _, molecule, atom = p
        p[0] = molecule.bind_atom(atom)

    def p_molecule_slash(self, p: tuple[Molecule, Molecule, str]):
        """molecule : molecule SLASH"""
        _, molecule, slash = p
        p[0] = molecule.push_bond(Bond.make(slash))

    def p_molecule_bond_atom(self, p: tuple[Molecule, Molecule, str]):
        """molecule : molecule BOND"""
        _, molecule, bond = p
        p[0] = molecule.push_bond(Bond.make(bond))

    def p_molecule_closure(self, p: tuple[Molecule, Molecule, str]):
    ...
    def p_molecule_submolecule(self, p: tuple[Molecule, Molecule, str, Molecule, str]):
    ...
    def p_submolecule_bond(self, p: tuple[Molecule, str, Molecule]):
